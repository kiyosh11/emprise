...
uint64_t encoded_bytes(WeightType type, uint64_t n) {
    ...
    case WeightType::q4_k:
        if (n % 256) throw std::runtime_error("Q4_K requires blocks of 256 elements");
        return checked_mul(n / 256, 144);
...
Gguf::Gguf(const std::filesystem::path& path): file_(path, std::ios::binary | std::ios::ate) {
    ...
    for (uint64_t i = 0; i < nt; ++i) {
        ...
        if ((t.type == WeightType::q6_k || t.type == WeightType::q4_k) && t.shape[0] % 256)
