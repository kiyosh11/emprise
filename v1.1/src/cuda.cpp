...
// Fused projection: up to four Q6_K matrices sharing the same input vector,
// computed in a single launch with the four-row-reuse decode. Row counts r0..r3
// (multiples of 4) partition the output rows in order; r2/r3 may be zero.
extern "C" __global__ void fused_q6_proj(const unsigned char* __restrict__ w0,const unsigned char* __restrict__ w1,
                                         ...
                                         int cols,int r0,int r1,int r2,int r3) {
    int row=blockIdx.x*4,lane=threadIdx.x;
    int b0=r0,b1=b0+r1,b2=b1+r2,b3=b2+r3;
    if(row>=b3) return;
    const unsigned char* w;
    float* y;
    int base,end;
    if(row<b0) { w=w0; y=y0; base=0; end=b0; }
    else if(row<b1) { w=w1; y=y1; base=b0; end=b1; }
    else if(row<b2) { w=w2; y=y2; base=b1; end=b2; }
    else { w=w3; y=y3; base=b2; end=b3; }
    int local=row-base;
    int i0=(lane*8)&255,h=i0>>7,g=(i0>>5)&3,l=i0&31;
    int loff=h*64+(g&1)*32+l, hoff=128+h*32+l;
    int shl=(g>=2)?4:0, shh=g*2, sci=h*8+g*2+(l>>4);
    ...
    const int nblocks=cols>>8, step=(blockDim.x*8)>>8;
    const float* xp=x+lane*8;
    int avail=end-local; if(avail>4) avail=4;
    float s0=0,s1=0,s2=0,s3=0;
    for(int block=(lane*8)>>8;block<nblocks;block+=step,xp+=blockDim.x*8) {
        const float4 xa=*(const float4*)xp, xb=*(const float4*)(xp+4);
        const unsigned char* bb=w+rs*local+(unsigned long long)block*216ull;
        s0+=q6_row(bb,loff,hoff,shl,shh,sci,xa,xb);
        if(avail>1) s1+=q6_row(bb+rs,loff,hoff,shl,shh,sci,xa,xb);
        if(avail>2) s2+=q6_row(bb+2*rs,loff,hoff,shl,shh,sci,xa,xb);
        if(avail>3) s3+=q6_row(bb+3*rs,loff,hoff,shl,shh,sci,xa,xb);
    }
    __shared__ float pr[4][128];
    pr[0][lane]=s0; if(avail>1)pr[1][lane]=s1; if(avail>2)pr[2][lane]=s2; if(avail>3)pr[3][lane]=s3;
    __syncthreads();
    ...
    if(!lane) {
        y[local]=pr[0][0];
        if(avail>1)y[local+1]=pr[1][0];
        if(avail>2)y[local+2]=pr[2][0];
        if(avail>3)y[local+3]=pr[3][0];
    }
}
...
class Cuda final: public Backend {
    ...
    void device_linear(const Tensor& t,Ptr w,Ptr x,Ptr y,bool reuse_quantized=false) {
        ...
        if(fast && t.type==WeightType::q6_k) {
            ...
            void* args[]={&w,&x,&y,&cols,&rows};
            check(launch(q4_f32_x4_kernel,(rows+3)/4,1,1,128,1,1,0,nullptr,args,nullptr),"launch Q4 FP32 x4 matvec");
        } else {
...
public:
    explicit Cuda(uint64_t requested, bool quantize): fast(quantize) {
        ...
        try {
            ...
            check(driver.symbol<int(*)(void**, void*, const char*)>("cuModuleGetFunction")(&q4_f32_x4_kernel, module, "linear_q4_f32_x4"), "get Q4 FP32 x4 kernel");
            ...
            check(driver.symbol<int(*)(void**, void*, const char*)>("cuModuleGetFunction")(&fused_proj_kernel, module, "fused_q6_proj"), "get fused projection kernel");
    ...
    void linear_many(Gguf& model, std::span<const LinearRequest> requests, std::span<const float> x) override {
        ...
        size_t offset=0;
        bool fused=false;
        if(!fast && all_q6 && requests.size()>=2 && requests.size()<=4) {
            bool ok=true;
            for(const auto& r:requests) if(r.tensor->shape[1]%4){ok=false;break;}
            if(ok) {
                Ptr w[4]={0,0,0,0}, yo[4]={0,0,0,0}; int rr[4]={0,0,0,0};
                size_t off=0;
                for(size_t i=0;i<requests.size();++i) {
                    w[i]=ptrs[i]; yo[i]=output+off*sizeof(float);
                    rr[i]=static_cast<int>(requests[i].tensor->shape[1]); off+=requests[i].output.size();
                }
                int c=static_cast<int>(x.size());
                void* args[]={&w[0],&w[1],&w[2],&w[3],&input,&yo[0],&yo[1],&yo[2],&yo[3],&c,&rr[0],&rr[1],&rr[2],&rr[3]};
                check(launch(fused_proj_kernel,static_cast<unsigned>((total+3)/4),1,1,128,1,1,0,nullptr,args,nullptr),"launch fused projection");
                fused=true;
            }
        }
        if(!fused) {
