...
| Weight format | Q4_K and Q6_K (tested end-to-end); FP32, FP16, BF16 decoding | Q4_K is the most common download |
...
|---|---|---:|---:|
| 9B Q4_K_M | Quadro P4000, 16 CPU threads | CUDA (device-resident) | ~21 tok/s |
...
9B Q4_K_M fits in about 6 GB where Q6_K needs closer to 8 GB. When the whole model
fits in VRAM, the engine keeps activations there too (device-resident path), which
removes the per-layer host round-trips.
...
- One model architecture (`qwen35`); Q4_K and Q6_K are the tested weight formats.
